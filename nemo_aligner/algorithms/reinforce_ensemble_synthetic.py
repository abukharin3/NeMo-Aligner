...
class ReinforceHackerSynthetic:
    ...
    def __init__(
        ...
        self.rm_critic_gt = rm_critic_gt
    ...
    def _run_inference(self, dataloader_builder, consumed_samples, is_validation):
        ...
        rollout_batches, futures_max, futures_min, futures_gt = [], [], [], []
        timer_metrics = {}
            ...
            for batch in batch_iterator:
                if not is_validation:
                    for _ in range(self.num_rollouts_per_prompt):
                        ...
                        futures_gt.append(self.rm_critic_gt.infer_rm_critic(rollout_batch, self.model))
                else:
                    rollout_batch = self.model.infer(batch)
                    rollout_batch["prompt_tokens"] = batch["text"] # Save prompt tokens for rloo
                    rollout_batches.append(rollout_batch)
        ...
        with reshard_context():
            ...
            for i in range(len(futures_max)):
                ...
                if self.cfg.ensemble_type == "mean":
                    rewards = (self.cfg.lam1 * rewards_max + self.cfg.lam2 * rewards_min) / 2
                elif self.cfg.ensemble_type == "std":
                    rewards = (self.cfg.lam1 * rewards_max + self.cfg.lam2 * rewards_min) - self.cfg.penalty * 0.5 *torch.abs(self.cfg.lam1 * rewards_max - self.cfg.lam2 * rewards_min)
